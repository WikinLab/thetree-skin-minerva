/*
 * the tree SSR preference bridge for the optional Vector content projection.
 *
 * The companion backend plugin owns only request-cookie -> skinData transport.
 * This adapter validates that transport, exposes the effective mode to the
 * skin, and persists a new cookie value before a deliberate full reload.
 */
...
const DEFAULT_CONTENT_PROJECTION_ENABLED = false;
...
export function resolveContentProjectionPreference(context = {}) {
  ...
  const available = payload?.protocol === CONTENT_PROJECTION_PROTOCOL && typeof payload.enabled === 'boolean';
  return Object.freeze({
    available,
    enabled: available ? payload.enabled : DEFAULT_CONTENT_PROJECTION_ENABLED,
    source: available ? 'ssr-plugin-cookie' : 'host-default'
  });
}
...
export function makeContentProjectionPersonalTool(context = {}) {
  ...
  const label = localizedLabel(preference.enabled, context.config?.lang || context.config?.['wiki.lang']);
  return Object.freeze({
...
export const theTreeContentProjectionPreferenceContract = Object.freeze({
  protocol: CONTENT_PROJECTION_PROTOCOL,
  serverDataKey: CONTENT_PROJECTION_SERVER_DATA_KEY,
  cookieName: CONTENT_PROJECTION_COOKIE_NAME,
  defaultEnabled: DEFAULT_CONTENT_PROJECTION_ENABLED,
  persistence: 'cookie-followed-by-full-reload',
  requires: 'thetree skinData plugin for SSR-stable per-browser selection'
});
