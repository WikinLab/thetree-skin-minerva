...
const DEFAULT_CONTENT_MODE = CONTENT_MODE_NATIVE;
...
export function resolveContentModePreference(context = {}) {
  ...
  return Object.freeze({
    available,
    mode: available && payload.enabled ? CONTENT_MODE_PROJECTED : DEFAULT_CONTENT_MODE,
    source: available ? 'ssr-plugin-cookie' : 'host-default'
  });
}

export function resolveContentProjectionPreference(context = {}) {
  const preference = resolveContentModePreference(context);
  return Object.freeze({
    available: preference.available,
    enabled: contentModeUsesProjection(preference.mode),
...
export function makeContentProjectionPersonalTool(context = {}) {
  ...
  const enabled = contentModeUsesProjection(preference.mode);
...
export function toggleTheTreeContentProjection(context = {}, storeState = {}) {
  ...
  if (!setContentProjectionPreference(!contentModeUsesProjection(preference.mode), storeState)) return false;
