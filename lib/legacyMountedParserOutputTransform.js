 ...
 */
import { projectParserOutputHtml } from './legacyParserOutputTransform';
import {
...
export const legacyMountedParserOutputTransformContract = Object.freeze({
  packageVersion: '0.0.355',
  featureBaseline: 'v355-template-literal-line-terminator-closure',
  currentIntegrationPhase: 'v355-template-literal-safe-host-source-role-and-nested-parser-output-projection',
  observeRootSelector: HOST_CONTENT_OBSERVE_ROOT_SELECTOR,
  categorySourceSelector: EDIT_PREVIEW_CATEGORY_SOURCE_SELECTOR,
  owns: [
    'all dynamic nested ParserOutput surfaces declared by the current immutable page projection contract',
    'fresh contract resolution on every transform run instead of a route-stale selector snapshot',
    'one host-content MutationObserver shared by exact interface roles and every nested ParserOutput child surface on the current page',
    'deterministic interface role and upstream primitive markers sourced only from the locked host contentName contract',
    'the same canonical projectParserOutputHtml compiler used by ordinary store-owned article output',
    'preview-local parser-output and catlinks surface markers without duplicate global ids'
  ],
  forbids: [
    'ordinary article WikiContent discovery or innerHTML rewriting',
    'contentName or viewName inspection outside the page contract',
    'page-specific visual values, interface CSS, or selector fallback synthesis',
    'alternate host selector fallback',
    'raw editor textarea, plugin tab, button, form, or tab-strip ParserOutput conversion',
    'page-specific MutationObserver instances',
    'Popups model or template synthesis inside the nested surface adapter'
  ]
});
