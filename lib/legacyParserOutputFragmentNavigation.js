 ...
 *
 * REL1_46 parser output relies on native same-document fragment navigation for
 ...
 * already browser-native fragment links. This bridge restores that ownership
 * boundary by letting hash-only links inside #mw-content-text .mw-parser-output
 * keep their native default action while stopping the host dynamic-content
...
function shouldPreserveNativeFragmentNavigation(event) {
...
export function createLegacyParserOutputFragmentNavigationRuntime(options = {}) {
  ...
  const listener = (event) => {
    if (!shouldPreserveNativeFragmentNavigation(event)) return;
...
export const legacyParserOutputFragmentNavigationContract = Object.freeze({
  phase: 'v246-mediawiki-fragment-navigation-contract',
  owns: [
    'hash-only anchors inside compiled MediaWiki parser-output',
    'native browser fragment target state required by upstream Cite :target CSS',
    'event ownership boundary between MediaWiki parser-output anchors and the tree dynamic-content router handling'
  ],
  forbids: [
    'synthetic Cite highlight classes',
    'manual scroll or focus replacement for same-document parser-output anchors',
    'document-title or fragment-target heuristics',
    'routing MediaWiki parser-output hash-only anchors through the host Vue router'
  ]
});
