...
function toHtmlString(value) {
  if (Array.isArray(value)) {
    return value.map((item) => toHtmlString(item)).join('');
  }
...
export default {
  ...
  methods: {
    clearRawHtmlFragment() {
      const nodes = this.__legacyRawHtmlFragmentNodes || [];
      for (const node of nodes) {
        if (node && node.parentNode) {
          node.parentNode.removeChild(node);
        }
      }
      this.__legacyRawHtmlFragmentNodes = [];
    },
    syncRawHtmlFragment() {
      this.clearRawHtmlFragment();
      ...
      this.__legacyRawHtmlFragmentNodes = nodes;
    }
  }
};
