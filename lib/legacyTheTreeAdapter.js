...
import { insertContentProjectionPersonalTool } from './adapters/thetree-content-projection';
...
export function makePersonalToolsItems(context = {}) {
  ...
  const withDarkMode = insertDarkModePersonalTool(baseItems, {
    theme: context.currentTheme || 'light',
    lang: context.config?.lang || context.config?.['wiki.lang'] || 'ko'
  });
  return normalizeAdapterItems(
    insertContentProjectionPersonalTool(withDarkMode, context),
    `pt-${accountType}`
  );
}
