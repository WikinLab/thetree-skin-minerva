const CONFIG_FALLBACKS = Object.freeze({
  ...
  logoLegacy1x: ['wiki.logo_url'],
  logoWordmark: ['skin.minerva.logo_wordmark'],
  siteName: ['wiki.site_name', 'wiki.name', 'site_name', 'siteName'],
  frontPage: ['wiki.front_page', 'front_page']
});

const NUMBER_CONFIG_FALLBACKS = Object.freeze({
  logoWordmarkWidth: ['skin.minerva.logo_wordmark_width'],
  logoWordmarkHeight: ['skin.minerva.logo_wordmark_height']
});
...
export function getMinervaConfiguredNumber(config = {}, name, fallback = null) {
  for (const key of NUMBER_CONFIG_FALLBACKS[name] || []) {
