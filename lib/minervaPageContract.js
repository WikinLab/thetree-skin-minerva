...
export function makeMinervaPageContract(context = {}) {
  ...
  const namespaceKind = !hasDocument ? 'special' : isTalkPage ? 'talk' : 'subject';
  const resolvedNamespaceId = !hasDocument ? -1 : namespaceId(document || {}, namespaceKind);
  const isCurrentRevision = !data.rev && !context.route?.query?.uuid;
  const canEdit = hasDocument && !data.error && permissionValue(data, 'edit', data.editable !== false);
