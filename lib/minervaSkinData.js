...
function logoData(config, siteName) {
  ...
  const width = getMinervaConfiguredNumber(config, 'logoWordmarkWidth', '');
  const height = getMinervaConfiguredNumber(config, 'logoWordmarkHeight', '');
  return {
    ...
    wordmark: src ? {
      ...
      width,
      height,
      style: width && height ? `width: ${width / 16}em; height: ${height / 16}em;` : ''
    } : null
  };
}
