...
import {
  getMinervaConfiguredBoolean,
  getMinervaConfiguredNumber,
  getMinervaConfiguredString
} from './minervaHostConfig.js';
import {
...
function logoData(config, siteName) {
  const legacy1x = getMinervaConfiguredString(config, 'logoLegacy1x', '');
  const src = getMinervaConfiguredString(config, 'logoWordmark', '');
  const width = getMinervaConfiguredNumber(config, 'logoWordmarkWidth', null);
  const height = getMinervaConfiguredNumber(config, 'logoWordmarkHeight', null);
  return {
    ...(legacy1x ? { '1x': legacy1x } : {}),
    'msg-sitetitle': siteName,
    wordmark: src && width && height ? {
      src,
      width,
      height,
...
export function makeMinervaSkinData(context = {}) {
  ...
  const logo = logoData(context.config || {}, siteName);
  return {
    ...title,
    ...titleMeta,
