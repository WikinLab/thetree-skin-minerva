...
function notificationButtons(context, personalItems) {
  const toolbarIds = new Set(['pt-notifications', 'pt-watchlist']);
  const buttons = personalItems
    .filter((item) => toolbarIds.has(item.id))
    .map((item) => button(context, item, ICON_BY_ITEM[item.id], 'minerva-notification'));
...
export function makeMinervaSkinData(context = {}) {
  ...
  const headerNotifications = notificationButtons(context, personalItems);
  const userMenuItems = personalItems.filter((item) => !['pt-notifications', 'pt-watchlist'].includes(item.id));
  const pageData = getLegacyPageData(context);
  ...
  return {
    ...
    'html-minerva-user-menu': personalMenu(context, userMenuItems),
    'data-minerva-notifications': headerNotifications,
