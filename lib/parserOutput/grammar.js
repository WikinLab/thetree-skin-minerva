...
export const PARSER_OUTPUT_FEATURE_MAP = Object.freeze({
  ...
  internalLink: freezeFeature({
    ...
    source: {
      system: 'thetree-namumark',
      rootClasses: ['wiki-link-internal'],
      missingClasses: ['not-exist'],
      selfClasses: ['wiki-self-link']
    },
    target: {
    ...
    equivalence: 'analog',
    loss: ['the tree current-page link class remains host-owned until a dedicated MediaWiki selflink mapping is ported']
  }),
  ...
  externalLink: freezeFeature({
    ...
    source: {
      system: 'thetree-namumark',
      rootClasses: ['wiki-link-external', 'wiki-link-whitelisted'],
      protocols: ['http', 'https', 'ftp']
    },
    target: { system: 'mediawiki-parser-output', rootClasses: ['external', 'text'] },
...
export const ARTICLE_INPUT_GRAMMAR = Object.freeze({
  ...
  ordinaryBlock: 'native block and inline nodes preserved unless an exact feature-map source rule applies',
...
export const ARTICLE_OUTPUT_GRAMMAR = Object.freeze({
  ...
  list: 'native ul/ol with semantic type/start attributes and without host presentation classes',
  indent: 'MediaWiki dl > dd indentation flow',
  blockquote: 'native MediaWiki blockquote',
  ordinaryBlock: 'native parser-output flow nodes preserved without wrapper insertion',
