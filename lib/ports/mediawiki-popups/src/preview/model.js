...
function makeTitleInExtractBold(extract, title) {
  const elements = [];
  const boldIdentifier = `<bi-${Math.random()}>`;
  const snip = `<snip-${Math.random()}>`;
  const normalizedTitle = String(title || '').replace(/\s+/g, ' ').trim();
  const escapedTitle = escapeRegExp(normalizedTitle);
  const regExp = new RegExp(`(^|\\s)(${escapedTitle})(|$)`, 'i');
  let normalizedExtract = String(extract || '').replace(/\s+/, ' ');
  normalizedExtract = normalizedExtract.replace(regExp, `$1${snip}${boldIdentifier}$2${snip}$3`);

  normalizedExtract.split(snip).forEach((part) => {
