...
function cssEntryImports(source, filename) {
  const imports = [];
  for (const line of source.split(/\r?\n/)) {
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith('/*') || trimmed.startsWith('*') || trimmed === '*/') continue;
    const match = /^@import\s+(["'])([^"']+)\1\s*;$/.exec(trimmed);
    if (!match) throw new Error(`Static stylesheet entry contains a non-import statement: ${filename}: ${trimmed}`);
...
function validateStylesheetDelivery(root, manifest) {
  ...
  if (!contract) return;
  if (contract.schema !== 1 || contract.mode !== 'single-static-entry') {
  ...
  const entry = normalizeRelativePath(contract.entry);
  const actual = cssEntryImports(fs.readFileSync(path.join(root, entry), 'utf8'), entry)
    .map((specifier) => normalizeRelativePath(path.posix.join(path.posix.dirname(entry), specifier)));
  const expected = [contract.originBundle, ...(contract.adapterImports || [])].map(normalizeRelativePath);
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(`Static stylesheet delivery order mismatch: expected [${expected.join(', ')}], found [${actual.join(', ')}]`);
  }
